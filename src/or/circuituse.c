                                ...
                                origin_circuit_t **circp)
{
  ...
  /*
  	...
  	if (get_options()->UseN23) {
  		log_warn(LD_BUG, "Ochir_N23--------------------------------------------- EXIT 1 : %s", conn->chosen_exit_name);
  	}
  */

    //Ochir_e2e
      	if (get_options()->UseE2E) {
      		log_warn(LD_BUG, "Ochir_N23-------------  chosen_exit_name: %s \n", conn->chosen_exit_name);
      	}
...
connection_ap_handshake_attach_circuit(entry_connection_t *conn)
{
  ...
  /*
    	//Ochir_N23
    	if (get_options()->UseN23) {
    		log_warn(LD_BUG, "Ochir_N23-------------------- EXIT 2 NAME %s,  ADDRESS %s:%d", conn->chosen_exit_name, safe_str_client(conn->socks_request->address),
    	           conn->socks_request->port);
    	}
  */
  tor_assert(base_conn->state == AP_CONN_STATE_CIRCUIT_WAIT);
