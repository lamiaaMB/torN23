...
static void command_process_e2eflowcontrol_cell(cell_t *cell, or_connection_t *conn);
...
#define PROCESS_CELL(tp, cl, cn) command_process_ ## tp ## _cell(cl, cn)
  ...
  switch (cell->command) {
    ...
    case CELL_FLOWCONTROL:
         ...
         // Ochir_e2e
       case CELL_E2EFLOWCONTROL:
         PROCESS_CELL(e2eflowcontrol, cell, chan);
         break;
    default:
...
/*ORs need to just relay credit cells = e2eflowcontrol_cell*/
static void
command_process_e2eflowcontrol_cell(cell_t *cell, or_connection_t *conn)
{
	  //Ochir_e2e
	    	if (get_options()->UseE2E) {
	    		log_warn(LD_BUG, "Ochir_E2E-------------  forward e2eflowcontrol_cell\n");
	    	}
...
{
    circuit_t *circ = circuit_get_by_circid_channel(cell->circ_id, conn);
    uint32_t cells_fwded_neighbor;
     ...
     * is now positive, but we've got nothing to send. */
    //make_circuit_active_on_conn(circ,conn);
    if (!circ->n_chan) { //if this is an exit, start reading streams again when a credit has been recieved
        /* IG: We may want to call this even if we're not the exit node,
         * for leaky-pipe reasons. */
       // circuit_resume_edge_reading(circ,NULL);
    }
