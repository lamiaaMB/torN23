...
// Ochir_E2E
void connection_ap_send_e2eflowcontrol(circid_t circ_id, edge_connection_t *conn);
void connection_ap_update_e2eflowcontrol(circid_t circ_id, edge_connection_t *conn, cell_t *cell);
int connection_ap_check_credit_e2eflowcontrol(streamid_t stream_id, circuit_t *circ);

//Ochir_E2E2
void connection_ap_send_e2estopsending(circid_t circ_id, edge_connection_t *conn);
void connection_ap_send_e2estartsending(circid_t circ_id, edge_connection_t *conn);
    ...
    /*
        //Ochir_N23
          	if (get_options()->UseN23) {
          		log_warn(LD_BUG, "Ochir_N23----------------------------------------- EXIT 2 ATTACH PENDING");
          	}*/
                               ...
                               int reason)
{
  ...
  if (!get_options()->LeaveStreamsUnattached || conn->use_begindir) {
    ...
    /*  //Ochir_N23
            	if (get_options()->UseN23) {
            		log_warn(LD_BUG, "Ochir_N23----------------------------------------- EXIT 2 DETACH");
            	}*/
                                            ...
                                            crypt_path_t *cpath)
{
  ...
  /*
    //Ochir_N23
             	if (get_options()->UseN23) {
             		log_warn(LD_BUG, "Ochir_N23----------------------------------------- EXIT 2 REWRITE AND ATTACH IF ALLOWED");
             	}*/
...
#define WARN_INTRVL_PRIV 300
    ...
    /*   //Ochir_N23
            	if (get_options()->UseN23) {
            		log_warn(LD_BUG, "Ochir_N23----------------------------------------- EXIT 2 REWRITE AND ATTACH");
            	}
   */
...
// Ochir_E2E2
/* Algorithm-2: TCP-inspired end-to-end flow control
 * This function allows to create and to send the "stop sending" cells from Client to Exit node.
 * Stop sending cell's type is RELAY_COMMAND_E2ESTOPSENDING as defined in or.h
 * It should be called when Client is not able to receive data at the same rate than sent from Exit*/
void
connection_ap_send_e2estopsending(circid_t circ_id, edge_connection_t *conn)
{

  int begin_type;
  char payload[CELL_PAYLOAD_SIZE]="";
  int payload_len;
  begin_type = RELAY_COMMAND_E2ESTOPSENDING;
  payload_len = (int)strlen(payload);

  connection_edge_send_command(conn, begin_type, payload, payload_len);
  if (get_options()->UseE2E2) {
	  log_warn(LD_BUG, "Ochir_E2E2------------- STOP SENT! STREAM %d\n", conn->stream_id);
  }
  return;
}
...
// Ochir_E2E2
/* Algorithm-2: TCP-inspired end-to-end flow control
 * This function allows to create and to send the "start sending" cells from Client to Exit node.
 * Start sending cell's type is RELAY_COMMAND_E2ESTARTSENDING as defined in or.h */
void
connection_ap_send_e2estartsending(circid_t circ_id, edge_connection_t *conn)
{

  int begin_type;
  char payload[CELL_PAYLOAD_SIZE]="";
  int payload_len;
  begin_type = RELAY_COMMAND_E2ESTARTSENDING;
  payload_len = (int)strlen(payload);
  log_warn(LD_BUG, "Ochir_E2E2------------- START SENding! STREAM %d\n", conn->stream_id);

  connection_edge_send_command(conn, begin_type, payload, payload_len);
  if (get_options()->UseE2E2) {
	  log_warn(LD_BUG, "Ochir_E2E2------------- START SENT! STREAM %d\n", conn->stream_id);
  }
  return;
}

// Ochir_E2E
/* Algorithm-1: credit based end-to-end flow control
 * This function allows to create and to send the credit cells from Client to Exit node.
 * Credit cell's type is RELAY_COMMAND_E2ECREDIT as defined in or.h */
void
connection_ap_send_e2eflowcontrol(circid_t circ_id, edge_connection_t *conn)
{
  int begin_type;
  char payload[CELL_PAYLOAD_SIZE]="";
  int payload_len;
  begin_type = RELAY_COMMAND_E2ECREDIT;
  payload_len = (int)strlen(payload);

  connection_edge_send_command(conn, begin_type, payload, payload_len);
  if (get_options()->UseE2E) {
	  log_warn(LD_BUG, "Ochir_E2E------------- CREDIT SENT! STREAM %d\n", conn->stream_id);
  }
  return;
}
...
// Ochir_E2E
/* Algorithm-1: credit based end-to-end flow control
 * This function allows to update the credit balance at Exit node.
 * Credit balance is a table defined in or.h, where the received credits are saved for each stream.
 * It is called when Exit node receives a credit cell from Client*/
void
connection_ap_update_e2eflowcontrol(circid_t circ_id, edge_connection_t *conn, cell_t *cell)
{
	uint8_t body = get_uint8(cell->payload);
	if (body == RELAY_COMMAND_E2ECREDIT){ // CREDIT_CELL
		// update credit
		int i;
		for (i=0; i<STREAME2E_TAB_MAX; i++)
		  {
			  if (i==conn->stream_id){
				  conn->on_circuit->e2e_credit_balance_n[i] += STREAME2E_INCREMENT;
				  log_warn(LD_BUG, "Ochir_E2E-------------  UPDATE CREDIT STREAM : %d, BALANCE: %d \n", i, conn->on_circuit->e2e_credit_balance_n[i]);
...
// Ochir_E2E
/* Algorithm-1: credit based end-to-end flow control
 * This function allows to check if there is enough credit for a given stream.
 * It returns 1 if credit is enough and 0 if no more credit for a specific stream*/
int
connection_ap_check_credit_e2eflowcontrol(streamid_t stream_id, circuit_t *circ)
{
	int i = stream_id;
	if (circ->e2e_credit_balance_n[i]>0)  {
		//log_warn(LD_BUG, "Ochir_E2E-------------  ENOUGH CREDIT STREAM : %d, BALANCE: %d \n", i, circ->e2e_credit_balance_n[i]);
		return 1;
	}
	else{
		//log_warn(LD_BUG, "Ochir_E2E-------------  NO CREDIT STREAM : %d, BALANCE: %d \n", i, circ->e2e_credit_balance_n[i]);
		return 0;
	}
}
...
connection_ap_handshake_send_begin(entry_connection_t *ap_conn)
{
  ...
  // Ochir_E2E credit init at client for memory
 /* int i;
  for (i=0; i<100000; i++)
  {
	  if (i==edge_conn->stream_id){
		  edge_conn->on_circuit->e2e_credit_balance_n[i] = STREAME2E_START;
		  log_warn(LD_BUG, "Ochir_E2E-------------  E2E_CREDIT STREAM : %d, BALANCE: %d \n", i, edge_conn->on_circuit->e2e_credit_balance_n[i]);

	  }
	  else
		  edge_conn->on_circuit->e2e_credit_balance_n[i] = 0;
  }
*/
  ...
  /*  if (get_options()->UseE2E) {
    			log_warn(LD_BUG, "Ochir_E2E------------- handshake send begin \n");
    		}*/
                        ...
                        int use_begindir, int want_onehop)
{
  ...
  /* //Ochir_N23
        	if (get_options()->UseN23) {
        		log_warn(LD_BUG, "Ochir_N23----------------------------------------- EXIT 2 MAKE LINK");
        	}*/
  ...
  if (rh.length > RELAY_PAYLOAD_SIZE)
	...
	/*//Ochir_N23
		if (get_options()->UseN23) {
			...
			log_warn(LD_BUG, "Ochir_N23-------------  EXIT_BEGIN_CONN CIRC: %u STREAM: %d COMMAND %d length %zu \n", circ->n_circ_id, rh.stream_id, rh.command, rh.length);
...
*/
  ...
  /*
      ...
      if (get_options()->UseN23) {
      			...
      			log_warn(LD_BUG, "Ochir_N23-------------  EXIT_BEGIN_CONN CIRC: %u STREAM: %d COMMAND %d length %zu ADDRESS %s : PORT %s\n", circ->n_circ_id, rh.stream_id, rh.command, rh.length, address, port);
...
connection_exit_connect_dir(edge_connection_t *exitconn)
{
  ...
  /*//Ochir_N23
    	if (get_options()->UseN23) {
    		log_warn(LD_BUG, "Ochir_N23------------------exit_connect_dir ADDRESS %s", dirconn->_base.address);
    	}*/
