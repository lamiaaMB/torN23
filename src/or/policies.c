	...
	/*
		//Ochir_N23
	  	if (get_options()->UseN23) {
	  		log_warn(LD_BUG, "Ochir_N23----------------------------------------- EXIT 1 NODE_POLICY, IS EXIT : %d, RI-EXIT_POLICY: %d, MD-EXIT POLICY %d, REJECT_ALL %d\n", node->is_exit, node->ri->exit_policy, node->md->exit_policy, node->rejects_all);
	  	}*/
  if (node->rejects_all)
    return ADDR_POLICY_REJECTED;
