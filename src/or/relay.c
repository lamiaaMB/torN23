...
relay_command_to_string(uint8_t command)
{
  ...
  switch (command) {
	...
	//Ochir_E2E - Algorithm-1: credit based end-to-end flow control
	case RELAY_COMMAND_E2ECREDIT: return "E2ECREDIT";
	//Ochir_E2E2 - Algorithm-2: TCP-inspired end-to-end flow control
	case RELAY_COMMAND_E2ESTOPSENDING: return "E2ESTOP";
	case RELAY_COMMAND_E2ESTARTSENDING: return "E2ESTART";
                              ...
                              const char *filename, int lineno)
{
	...
	//Ochir_E2E2 - Algorithm-2: TCP-inspired end-to-end flow control
		/*if (get_options()->UseE2E2) {
			log_warn(LD_BUG, "Ochir_E2E2------------- OUTGOING COMMAND %s, STREAM %d \n", relay_command_to_string(relay_command), stream_id);
		}*/


		//Ochir_E2E - Algorithm-1: credit based end-to-end flow control
		if (get_options()->UseE2E) {

			if (CIRCUIT_IS_ORIGIN(circ)) // client?
			{
				FILE* fp;
				fp = fopen("/home/lamiaa/new/private-tor/c6/tor.output", "a");
				if (fp != NULL) {
				struct timeval tp;
				...
				if (relay_command == 2){ // DATA
					//log_warn(LD_BUG, "Ochir_E2E-------------  EXIT OUTGOING CIRC: %u STREAM: %d COMMAND %s(%d) length %zu \n", circ->n_circ_id, stream_id, relay_command_to_string(relay_command), relay_command, payload_len);

					// check credit
					if (connection_ap_check_credit_e2eflowcontrol(stream_id, circ)==1) { // enough credit

						// send DATA and credit--
//						log_warn(LD_BUG, "Ochir_E2E-------------  ENOUGH CREDIT = DATA SENT! STREAM: %d COMMAND %s(%d) length %zu \n",stream_id, relay_command_to_string(relay_command), relay_command, payload_len);

						// udpate credit
						circ->e2e_credit_balance_n[stream_id]--;
//						log_warn(LD_BUG, "Ochir_E2E-------------  CREDIT BALANCE STREAM: %d NEW BALANCE: %d\n", stream_id, circ->e2e_credit_balance_n[stream_id]);

					}
					else {
						// don't send DATA
//						log_warn(LD_BUG, "Ochir_E2E-------------  NO CREDIT = DON'T SEND DATA! STREAM: %d COMMAND %s(%d) length %zu, before blocked %d \n", stream_id, relay_command_to_string(relay_command), relay_command, payload_len, circ->streams_blocked_on_n_chan);

						if (circ->streams_blocked_on_n_chan !=1) {
							circ->streams_blocked_on_n_chan=1; // exit
//							log_warn(LD_BUG, "Ochir_E2E-------------  NO CREDIT = DON'T SEND DATA! STREAM: %d COMMAND %s(%d) length %zu, after blocked %d \n", stream_id, relay_command_to_string(relay_command), relay_command, payload_len, circ->streams_blocked_on_n_chan);
						}
						return 0;
					}
				}
		...
		//Ochir_E2E2 - Algorithm-2: TCP-inspired end-to-end flow control
	if(relay_command== RELAY_COMMAND_E2ESTARTSENDING){
    	if (get_options()->UseE2E2) {
			log_warn(LD_BUG, "Ochir_E2E2-------------  RECEIVED START STREAM %d\n", stream_id);
			//connection_ap_send_e2estopsending(circ->n_circ_id, conn); // for test

			// unblock stream
			circ->streams_blocked_on_n_chan=0;
			log_warn(LD_BUG, "Ochir_E2E2-------------  UNBLOCK STREAM: %d blocked %d \n", stream_id, circ->streams_blocked_on_n_chan);
  ...
  //Ochir_E2E2 - Algorithm-2: TCP-inspired end-to-end flow control
	if ( relay_command ==  RELAY_COMMAND_E2ESTOPSENDING){
		if (get_options()->UseE2E2) {
			log_warn(LD_BUG, "Ochir_E2E2-------------  RECEIVED STOP STREAM %d\n", stream_id);
			//connection_ap_send_e2estartsending(circ->n_circ_id, conn); // for test

			// unblock stream
			circ->streams_blocked_on_n_chan=1;
			log_warn(LD_BUG, "Ochir_E2E2-------------  BLOCK STREAM: %d blocked %d \n", stream_id, circ->streams_blocked_on_n_chan);
...
//  log_fn(LOG_DEBUG,"command %d stream %d", rh.command, rh.stream_id);
  ...
  if (get_options()->UseE2E2) {
	  ...
	  if (CIRCUIT_IS_ORIGIN(circ)){
		  //log_warn(LD_BUG, "Ochir_E2E2-------------  INCOMING COMMAND %s, STREAM %d\n", relay_command_to_string(rh.command), conn->stream_id);
		  if (rh.command == RELAY_COMMAND_CONNECTED){ // if received RELAY_COMMAND_CONNECTED cell
		  		if (layer_hint) { // if client
		  			// SEND START SENDING
		  			connection_ap_send_e2estartsending(circ->n_circ_id, conn);
					  conn->base_.stopped_sending = 0;

		  		}
		  }
		  if (rh.command == RELAY_COMMAND_DATA) { // if received DATA cell
			  if (layer_hint) { // if client

				  circ->e2e_recv_data_n[conn->stream_id]+=1;
				 // log_warn(LD_BUG, "Ochir_E2E2-------------  DATA RECEIVED %d STREAM %d SIZE %d\n", circ->e2e_recv_data_n[conn->stream_id], conn->stream_id, rh.length);

				  // check if buffer full ?
if(conn->base_.type == CONN_TYPE_AP){
				  buf_t *out =buf_copy( conn->base_.outbuf);
				  buf_t *in = conn->base_.inbuf;

				  if(buf_datalen(out) >= 5000 && conn->base_.stopped_sending == 0){
					  for ( int i = 0; i < buf_datalen(out); i++){
						  char* header = buf_read_header(out) ;//= chunk_copy(out->head);

						  log_warn(LD_BUG, "Lamiaa_E2E2------------- DATA RECEIVED %s inbuf = %d outbuf = %d\n", header,buf_datalen(in),buf_datalen(conn->base_.outbuf));
					  }
//					  connection_ap_send_e2estopsending(circ->n_circ_id, conn);
//					  conn->base_.stopped_sending = 1;
				  }
				  //ex. conn->outbuf_flushlen > 10*CELL_PAYLOAD_SIZE
				  //ex. conn->n_read : control_event_stream_bandwidth_used()

				  // if full, send stop sending cell
}
			  }
		  }
	  }
  	...
  	if (get_options()->UseE2E) {
  		...
  		if (CIRCUIT_IS_ORIGIN(circ)){
			...
			fclose(fp);
  			...
  			if (rh.command == RELAY_COMMAND_CONNECTED){ // if received RELAY_COMMAND_CONNECTED cell
				if (layer_hint) // if client Mashael:note that this will be true in case of dir server circuits.
				  connection_ap_send_e2eflowcontrol(circ->n_circ_id, conn); // 1st sent

			}

  			if (rh.command == RELAY_COMMAND_DATA){ // if received DATA cell
  				if (layer_hint){ // if client
//Mashael: this part is buggy. When a credit cell is sent (second time), a seg fault occurs at the client.
//  					log_warn(LD_BUG, "Ochir_E2E-------------  Layer Hint %d STREAM %d SIZE %d\n", circ->e2e_recv_data_n[conn->stream_id], conn->stream_id, rh.length);
  					circ->e2e_recv_data_n[conn->stream_id]+=1;
  					circ->e2e_enough_recv_n[conn->stream_id]+=1;

//  					log_warn(LD_BUG, "Ochir_E2E-------------  DATA RECEIVED %d STREAM %d SIZE %d\n", circ->e2e_recv_data_n[conn->stream_id], conn->stream_id, rh.length);

  					if (circ->e2e_enough_recv_n[conn->stream_id]== STREAME2E_RECV_MAX){
//  						log_warn(LD_BUG, "Ochir_E2E-------------  SEND CREDIT AGAIN! STREAM %d\n", conn->stream_id);
  						connection_ap_send_e2eflowcontrol(circ->n_circ_id, conn);
  						circ->e2e_enough_recv_n[conn->stream_id]=0;
  					}
  				}
  			}
  		}
  ...
  //Ochir_E2E2 - Algorithm-2: TCP-inspired end-to-end flow control
	case RELAY_COMMAND_E2ESTARTSENDING:
    	if (get_options()->UseE2E2) {
			log_warn(LD_BUG, "Ochir_E2E2-------------  RECEIVED START STREAM %d\n", conn->stream_id);
			//connection_ap_send_e2estopsending(circ->n_circ_id, conn); // for test
			...
			// unblock stream
			circ->streams_blocked_on_n_chan=0;
			log_warn(LD_BUG, "Ochir_E2E2-------------  UNBLOCK STREAM: %d blocked %d \n", conn->stream_id, circ->streams_blocked_on_n_chan);

		}
    	return 0;

  //Ochir_E2E2 - Algorithm-2: TCP-inspired end-to-end flow control
	case RELAY_COMMAND_E2ESTOPSENDING:
		if (get_options()->UseE2E2) {
			log_warn(LD_BUG, "Ochir_E2E2-------------  RECEIVED STOP STREAM %d\n", conn->stream_id);
			//connection_ap_send_e2estartsending(circ->n_circ_id, conn); // for test
  ...
  //Ochir_E2E - Algorithm-1: credit based end-to-end flow control
  case RELAY_COMMAND_E2ECREDIT:
  	if (get_options()->UseE2E) {
//			log_warn(LD_BUG, "Ochir_E2E-------------  CREDIT RECEIVED STREAM %d\n", conn->stream_id);
			// update credit balance
			connection_ap_update_e2eflowcontrol(circ->n_circ_id, conn, cell);
			// unblock stream
			if (circ->streams_blocked_on_n_chan!=0) {
				circ->streams_blocked_on_n_chan=0;
				log_warn(LD_BUG, "Ochir_E2E-------------  UNBLOCK STREAM: %d blocked %d \n", conn->stream_id, circ->streams_blocked_on_n_chan);
			}
...
//
//
//      // Ochir_E2E credit init
//    	  /*int i, E2E_N2=20, E2E_N3=30;
//    	  for (i=0; i<100000; i++)
//    	  {
//    		  if (i==conn->stream_id){
//    			  conn->on_circuit->e2e_credit_balance_n[i] = STREAME2E_START;
//    			  log_warn(LD_BUG, "Ochir_E2E-------------  E2E_CREDIT STREAM : %d, BALANCE: %d \n", i, conn->on_circuit->e2e_credit_balance_n[i]);
//
//    		  }
//    		  else
//    			  conn->on_circuit->e2e_credit_balance_n[i] = 0;
//    	  }*/
//
//          return connection_exit_begin_conn(cell, circ);
        case RELAY_COMMAND_DATA:
...
//      		log_warn(LD_BUG,"Ochir_N23--- command %d stream %d ", rh.command, rh.stream_id);
//      		log_warn(LD_BUG, "Ochir_N23--------------------------------------------- DATA CELL");
//      		 if (( layer_hint && --layer_hint->deliver_window < 0) ||
//      		              (!layer_hint && --circ->deliver_window < 0)) {
//      	          circuit_consider_sending_sendme(circ, layer_hint);
//
//      		 }
//      	}
          // Lamiaa : commented the if condition to make it possible to download large files

//    if (!get_options()->UseN23) {
          if (( layer_hint && --layer_hint->deliver_window < 0) ||
              (!layer_hint && --circ->deliver_window < 0)) {
            log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
                   "(relay data) circ deliver_window below 0. Killing. N23");
            if (conn) {
              /* XXXX Do we actually need to do this?  Will killing the circuit
               * not send an END and mark the stream for close as appropriate? */
              connection_edge_end(conn, END_STREAM_REASON_TORPROTOCOL);
...
//    }
          ...
          connection_write_to_buf((char*)(cell->payload + RELAY_HEADER_SIZE),
                                  rh.length, TO_CONN(conn));
//          log_warn(LD_BUG, "Lamiaa_E2E2-------------  Flushing Output : %d  \n",  buf_datalen(conn->base_.outbuf));

            //Lamiaa: IF using E2E2 after freeing space in outbuf, start sending again
            if (conn->base_.type == CONN_TYPE_AP){
        if(get_options()->UseE2E2  && buf_datalen(conn->base_.outbuf) < 6000){
        	// SEND START SENDING
        if(conn->base_.stopped_sending ==1){
//        	  log_warn(LD_BUG, "Lamiaa_E2E2-------------  Connection Type : %d\n", conn->base_.type);

        			  			connection_ap_send_e2estartsending(0, conn);
        			  			conn->base_.stopped_sending =0;
        }
        }
            }
                                  ...
                                  int *max_cells)
{
 ...
 repeat_connection_edge_package_raw_inbuf:
  ...
  log_debug(domain,"circ->package_window is now %d",circ->package_window);
...
make_circuit_active_on_conn(circuit_t *circ, or_connection_t *conn)
{
  ...
  if (circ->n_chan == conn) {
//    add_cell_ewma_to_conn(conn, &circ->n_chan_cells);
  } else {
    or_circuit_t *orcirc = TO_OR_CIRCUIT(circ);
    tor_assert(conn == orcirc->p_chan);
//    add_cell_ewma_to_conn(conn, &orcirc->p_chan_cells);
  }
